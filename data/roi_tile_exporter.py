...
sys.path.insert(1, '/Users/gbatch/Documents/projects/current/patch_extraction_scripts_andrea')
from data.images.wsi_reader import WSIReader # from data.images.wsi_reader
from data.contours import read_annotations, contour_to_mask, check_relative_rect_positions
...
class ROITileExporter:
    ...
    def __init__(self, data_dir, slide_id, experiment_name='', annotations_dirname='', tile_size=1024, mpp=0.2,
        ...
        try:
            print('debugging 7') # check what we can see from None
            print(f"\tFiles visible from {self.data_dir} (all)\n\t\t",
                  tuple(self.data_dir.glob('*')))
            print(f"\tFiles visible from {self.data_dir} (matching '*.ndpi')\n\t\t",
                  tuple(self.data_dir.glob('*.ndpi')))
            print(f"\tFiles visible from {self.data_dir} (matching '*/*.ndpi')\n\t\t",
                  tuple(self.data_dir.glob('*/*.ndpi')))

            self.slide_path = next(path for path in chain(tuple(self.data_dir.glob('*.tiff')),
                                                          tuple(self.data_dir.glob('*.svs')),
                                                          tuple(self.data_dir.glob('*.ndpi')),
                                                          tuple(self.data_dir.glob('*/*.tiff')),
                                                          tuple(self.data_dir.glob('*/*.svs')),
    ...
    def make_segmentation_map(self, tile_contours, labels, mask_origin):
        ...
        order = tuple(self.label_values)  # get the label names
        ...
        for contour, label in zip(tile_contours, labels):
            try:
                mask = contour_to_mask(
                    contour, value=self.label_values[label], shape=shape, mask=mask, mask_origin=mask_origin
