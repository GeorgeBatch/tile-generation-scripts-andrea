...
class BaseWSIReader:
    ...
    def read_region(self, x_y, level, size, base_level_coordinates=True):
        pass
...
class BioFormatsReaderBase(BaseWSIReader):
    ...
    def read_region(self, x_y, level, size):
        x, y = x_y
        tile_w, tile_h = size
        width, height = self.get_dimensions(level)
        tile_w = width - x if (x + tile_w > width) else tile_w
