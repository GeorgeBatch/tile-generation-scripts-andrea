...
import time
import tqdm
from openslide import OpenSlide, PROPERTY_NAME_MPP_X, PROPERTY_NAME_MPP_Y, OpenSlideError
import numpy as np
import cv2
from skimage.morphology import remove_small_objects, remove_small_holes

# custom imports from the scripts in the directory
from .base_wsi_reader import TiffBaseReader

# not there
# from base.utils import utils, debug

# -----------------------------------------------------------------------------
class WSIReader(TiffBaseReader):

    # can use this pattern in all objects with intrinsic options in order to use them more flexibily in scripts
    @staticmethod
    ...
    def get_reader_options(include_path=True, include_thresholds=True, args=()):
        ...
        parser = WSIReader.add_reader_args(parser, include_path, include_thresholds)
        if args:
    ...
    def read_locations(self, qc_store=None):
        ...
        name_ext = re.sub('\.(ndpi|svs)', '.json', os.path.basename(self.file_name))
        # debugging 8
        print('debugging 8') # check what we can see from None
        ...
        if qc_store:
            ...
            print('\toriginal slide_loc_path:', slide_loc_path)
            # trying to make it work
            #slide_loc_path = os.path.join(self.opt.data_dir, 'data', 'wsi_annotations', name_ext)
            #print('\tproposed slide_loc_path:', slide_loc_path)
            ...
            with open(slide_loc_path, 'r') as slide_loc_file:
                ...
                print('debugging 10')
                print('\t', quality_control.get('mpp_x', 'Key not found'))
    ...
    def is_HnE(image, threshold=0.4, sat_thresh=20, min_size=50, area_threshold=200):
        ...
        if image.shape[-1] == 4:
            ...
            image = image[..., :3]
        hsv_image = cv2.cvtColor(image.astype(np.float32), cv2.COLOR_RGB2HSV).astype(float)
        # saturation check
